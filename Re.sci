...
//μ (nu) == ν/ρ is the dynamic viscosity of the fluid (Pa·s or N·s/m2 or kg/(m·s)) []
//ν (ni) is the kinematic viscosity of the fluid (m2/s) [3]
...
function [Re]=Re(varargin)
    ...
    if rhs==3 then
        ...
        mprintf(gettext('Kinematic viscosity of the fluid ν: %f [m2/s]]\n\n'),varargin(3));
    end
    if rhs==4 then
        U=varargin(1);
        L=varargin(2);
        rho=varargin(3);
        ni=varargin(4)/varargin(3);
        mprintf(gettext('Flow speed U: %f [m/s]\n'),varargin(1));
        mprintf(gettext('Characteristic linear dimension L: %f [m]\n'),varargin(2));
        mprintf(gettext('Density of the fluid ρ: %f [kg/m3]\n'),varargin(3));
        mprintf(gettext('Dynamic viscosity of the fluid μ: %f [Pa·s or N·s/m2 or kg/(m·s)]\n'),varargin(4));
        mprintf(gettext('( Kinematic viscosity of the fluid ν: %f [m2/s]] )\n\n'),ni);
    end
    Re = U*L/ni;
